...
class DeiTSuper(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # configs for the sampled subDeiT
        ...
        for i in range(depth):
            self.blocks.append(DeiTEncoderLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
        ...
        self.dist_token_1 = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.dist_token_2 = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.dist_options = {
            0:self.dist_token,
            1:self.dist_token_1,
            2:self.dist_token_2
        ...
        trunc_normal_(self.dist_token_1, std=.02)
        trunc_normal_(self.dist_token_2, std=.02)
        ...
        # classifier head
        self.head_dist = LinearSuper(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def set_sample_config(self, config: dict):
        self.sample_dist_choice = config['dist_choice']
        ...
        self.head_dist.set_sample_config(self.sample_embed_dim[-1], self.num_classes)
    ...
    def get_sampled_params_numel(self, config):
        ...
        return sum(numels) + self.sample_embed_dim[0]* (4 +self.patch_embed_super.num_patches)#pos_embed + cls_token + dist_token
    def get_complexity(self, sequence_length):
        ...
        for blk in self.blocks:
            total_flops +=  blk.get_complexity(sequence_length+2)
        total_flops += self.head.get_complexity(sequence_length+2)
        total_flops += self.head_dist.get_complexity(sequence_length + 2)
        return total_flops
    def forward_features(self, x):
        B = x.shape[0]
        x = self.patch_embed_super(x)
        cls_tokens = self.cls_token[..., :self.sample_embed_dim[0]].expand(B, -1, -1)
        dist_tokens = self.dist_options[self.sample_dist_choice][..., :self.sample_embed_dim[0]].expand(B, -1, -1)
        ...
        if self.gp:
            return torch.mean(x[:, 2:] , dim=1), x[:, 1]
...
class DeiTEncoderLayer(nn.Module):
    """Encoder layer block.
    ...
    def get_complexity(self, sequence_length):
        ...
        if self.is_identity_layer:
            return total_flops
        total_flops += self.attn_layer_norm.get_complexity(sequence_length+2)
        total_flops += self.attn.get_complexity(sequence_length+2)
        total_flops += self.ffn_layer_norm.get_complexity(sequence_length+2)
        total_flops += self.fc1.get_complexity(sequence_length+2)
        total_flops += self.fc2.get_complexity(sequence_length+2)
        return total_flops
