...
from model.supernet_DeiT import DeiTSuper
import argparse
import os
import yaml
...
def decode_cand_tuple(cand_tuple):
    depth = cand_tuple[0]
    return depth, list(cand_tuple[1:depth+1]), list(cand_tuple[depth + 1: 2 * depth + 1]), cand_tuple[-2],cand_tuple[-1]
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args, device, model, model_without_ddp, choices, val_loader, test_loader, output_dir):
        ...
        self.penalty_num = args.penalty_num
    ...
    def is_legal(self, cand):
        ...
        if 'visited' in info:
            return False
        depth, mlp_ratio, num_heads, dist_choice,embed_dim = decode_cand_tuple(cand)
        sampled_config = {}
        sampled_config['layer_num'] = depth
        sampled_config['mlp_ratio'] = mlp_ratio
        sampled_config['num_heads'] = num_heads
        sampled_config['dist_choice'] = dist_choice
    ...
    def get_random_cand(self):
        ...
        cand_tuple.append(random.choice(self.choices['dist_choice']))
    ...
    def get_mutation(self, k, mutation_num, m_prob, s_prob):
        ...
        def random_func():
            ...
            random_s = random.random()
            if random_s < s_prob:
                dist_choice = random.choice(self.choices['dist_choice'])
            ...
            result_cand = [depth] + mlp_ratio + num_heads + [dist_choice] + [embed_dim]
    ...
    def search(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            for i, cand in enumerate(self.keep_top_k[50]):
                if self.vis_dict[cand]['params'] < self.parameters_limits:
                    count += 1
                if self.vis_dict[cand]['params'] > params_max:
                    params_max = self.vis_dict[cand]['params']
            for cand in self.candidates:
                if self.vis_dict[cand]['params'] > params_max:
                    params_max = self.vis_dict[cand]['params']
            print("count:{}".format(count))
            print("param_max:{}".format(params_max))
            self.update_top_k(
                self.candidates, k=self.select_num, key=lambda x: (count/50)*self.vis_dict[x]['acc']-(1-count/50)*100*((max(self.vis_dict[x]['params'],self.parameters_limits)-self.parameters_limits)/abs(params_max-self.parameters_limits)))
            self.update_top_k(
                self.candidates, k=50, key=lambda x: (count/50)*self.vis_dict[x]['acc']-(1-count/50)*100*((max(self.vis_dict[x]['params'],self.parameters_limits)-self.parameters_limits)/abs(params_max-self.parameters_limits)))
...
def get_args_parser():
    ...
    parser.add_argument('--penalty-num', type=float, default=1)
...
def main(args):
    ...
    print(f"Creating SuperDeiT")
    print(cfg)
    model = DeiTSuper(img_size=args.input_size,
    ...
    choices = {'num_heads': cfg.SEARCH_SPACE.NUM_HEADS, 'mlp_ratio': cfg.SEARCH_SPACE.MLP_RATIO,
               'embed_dim': cfg.SEARCH_SPACE.EMBED_DIM , 'depth': cfg.SEARCH_SPACE.DEPTH,'dist_choice':cfg.SEARCH_SPACE.DIST_CHOICE}
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('AutoDeiT evolution search', parents=[get_args_parser()])
